...
class Settings(BaseSettings):
    ...
    provider_rate_limit: int | None = Field(default=None, validation_alias="PROVIDER_RATE_LIMIT")
    provider_rate_window: float | None = Field(
        default=None, validation_alias="PROVIDER_RATE_WINDOW"
    )
    provider_max_concurrency: int = Field(
        default=5, validation_alias="PROVIDER_MAX_CONCURRENCY"
    )
    provider_fallbacks: dict[str, list[str]] = Field(
        default_factory=dict, validation_alias="PROVIDER_FALLBACKS"
    )
    ...
    @field_validator("model", "model_opus", "model_sonnet", "model_haiku", "model_fable")
    @classmethod
