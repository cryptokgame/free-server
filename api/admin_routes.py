...
async def test_provider(provider_id: str, payload: AdminConfigPayload, request: Request):
    require_loopback_admin(request)
    settings = get_cached_settings()
    
    # Merge existing settings with incoming values for testing
    values = _filtered_values(payload.values)
    config = load_config_response()
    current_fields = {field["key"]: field["value"] for field in config["fields"]}
    merged = {**current_fields, **values}
    
    from providers.registry import create_test_provider
    from config.provider_catalog import PROVIDER_CATALOG
    
    descriptor = PROVIDER_CATALOG.get(provider_id)
    if not descriptor:
        return {"provider_id": provider_id, "ok": False, "error_type": "UnknownProvider"}
        
    # Find the api_key and base_url from merged values
    api_key = ""
    if descriptor.credential_env:
        api_key = merged.get(descriptor.credential_env, "")
        if api_key == MASKED_SECRET:
            api_key = current_fields.get(descriptor.credential_env, "")
            
    base_url = None
    if descriptor.base_url_attr:
        base_url = merged.get(descriptor.base_url_attr.upper())
        
    try:
        provider = create_test_provider(provider_id, api_key, base_url, settings)
...
def _local_provider_url(provider_id: str, values: dict[str, str]) -> str:
    ...
    if provider_id == "ollama":
        base_url = values.get("OLLAMA_BASE_URL", "").strip()
        if base_url.endswith("/api"):
            base_url = base_url[:-4] + "/v1"
        elif base_url and not base_url.endswith("/v1") and not base_url.endswith("/v1/"):
