...
@router.post("/v1/test_provider")
async def test_provider_endpoint(
    request_data: TestProviderRequest,
    settings: Settings = Depends(get_settings),
    _auth=Depends(require_api_key),
):
    """Test if a provider API key is valid by querying its models list."""
    try:
        t0 = time.time()
        ...
        models = await provider.list_model_ids()
