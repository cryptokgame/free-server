...
class ClaudeProxyService:
    ...
    async def create_message(self, request_data: MessagesRequest) -> object:
        ...
        try:
            ...
            candidates = [routed.resolved]
            fallback_refs = self._settings.provider_fallbacks.get(request_data.model, [])
            if not fallback_refs:
                fallback_refs = self._settings.provider_fallbacks.get(routed.resolved.provider_model_ref, [])
                
            for ref in fallback_refs:
                try:
                    candidates.append(self._model_router.resolve(ref))
                except Exception as e:
                    logger.warning("Could not resolve fallback model '{}': {}", ref, e)

            last_exc = None
            for idx, candidate in enumerate(candidates):
                try:
                    if idx > 0:
                        logger.info("Trying fallback model: {}", candidate.provider_model_ref)
                        routed.request.model = candidate.provider_model
                        routed = type(routed)(request=routed.request, resolved=candidate)
                    ...
                    with logger.contextualize(request_id=request_id):
                        ...
                        try:
                            first_item = await anext(streamed)
                        except StopAsyncIteration:
                            async def _empty() -> AsyncIterator[str]:
                                if False: yield ""
                            return anthropic_sse_streaming_response(_empty())
                            
                        async def _rest(first: str, gen: AsyncIterator[str]) -> AsyncIterator[str]:
                            yield first
                            async for item in gen:
                                yield item
                                
                        return anthropic_sse_streaming_response(_rest(first_item, streamed))

                except ProviderError as e:
                    logger.warning("Provider {} failed: {}", candidate.provider_id, e)
                    last_exc = e
                except Exception as e:
                    logger.warning("Provider {} unexpected failure: {}", candidate.provider_id, e)
                    last_exc = e
