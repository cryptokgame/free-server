...
def load_config_response() -> dict[str, Any]:
    ...
    for field in FIELDS:
        ...
        if not str(raw_value).strip() and field.secret and field.section_id == "providers":
            from config.provider_catalog import PROVIDER_CATALOG
            from providers.registry import _credential_list_for
            for p_id, desc in PROVIDER_CATALOG.items():
                if desc.credential_env == field.key:
                    keys = _credential_list_for(desc, "")
                    if keys:
                        raw_value = keys[0]
                    break
...
def provider_config_status(
    ...
    for provider_id, descriptor in PROVIDER_CATALOG.items():
        ...
        from providers.registry import _credential_list_for
        keys = _credential_list_for(descriptor, value)
