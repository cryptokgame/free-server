...
function renderProviders(providerStatus) {
  ...
  providerStatus.forEach((provider) => {
    ...
    button.textContent = provider.kind === "local" ? "Test" : "Test Key";
...
function inputForField(field) {
  ...
  if (field.type === "secret_textarea") {
    const textarea = document.createElement("textarea");
    textarea.placeholder = field.configured
      ? "Configured - enter new keys (one per line) to replace"
      : "Not configured (enter one key per line)";
    textarea.value = "";
    textarea.dataset.secret = "true";
    return textarea;
  }
