...
def _claude_child_env(
    ...
    # Revisar si el usuario ya tiene sesión iniciada en Claude
    has_oauth = False
    try:
        import json
        from pathlib import Path
        ...
        if claude_json.exists():
            data = json.loads(claude_json.read_text("utf-8"))
            if "primaryToken" in data or "oauthToken" in data:
                has_oauth = True
    except Exception:
        pass
    ...
    if token := settings.anthropic_auth_token.strip():
        ...
        # Inject a dummy API key so Claude Code skips the OAuth login screen
        # Only do this if they don't have an OAuth token, otherwise Claude throws an error.
        env["ANTHROPIC_API_KEY"] = "sk-ant-api03-freecc"
...
def _ensure_claude_onboarded() -> None:
    """Ensure ~/.claude.json has hasCompletedOnboarding=true to bypass the login screen."""
    import json
    from pathlib import Path
    ...
    try:
        ...
        if not data.get("hasCompletedOnboarding"):
            ...
            claude_json.write_text(json.dumps(data, indent=2), "utf-8")
    except Exception:
        pass

def launch_claude(argv: Sequence[str] | None = None) -> None:
    """Launch Claude Code with Free Claude Code proxy environment variables."""
    _ensure_claude_onboarded()

    settings = get_settings()
    proxy_root_url = local_proxy_root_url(settings)
    if error := _preflight_proxy(proxy_root_url):
        print("El proxy no está corriendo. Iniciándolo automáticamente en segundo plano...", file=sys.stderr)
        
        # Start fcc-server in the background
        server_command = shutil.which("fcc-server")
        if not server_command:
            server_command = [sys.executable, "-m", "cli.entrypoints", "server"]
        else:
            server_command = [server_command]
            
        log_dir = Path.home() / ".fcc"
        ...
        for _ in range(50):
            ...
            print("Error: No se pudo iniciar el servidor automáticamente. Revisa ~/.fcc/server.log", file=sys.stderr)
            raise SystemExit(1)
