...
class ProviderConfig(BaseModel):
    ...
    def get_api_key(self) -> str:
        """Return the CURRENT API key (Fill first strategy)."""
        if not self.api_keys:
            return self.api_key
        return self.api_keys[self._key_index]

    def mark_key_exhausted(self) -> bool:
        """Rotate to the next API key. Returns True if a new key is available, False if exhausted all."""
        if not self.api_keys or len(self.api_keys) <= 1:
            return False
        
        next_index = self._key_index + 1
        if next_index >= len(self.api_keys):
            return False # We have exhausted the entire pool of keys
            
        object.__setattr__(self, "_key_index", next_index)
        return True
