...
def _create_gemini(config: ProviderConfig, _settings: Settings) -> BaseProvider:
    ...
    # Forzar empíricamente los límites de la capa gratuita (10 RPM, Ventana de 60s)
    config.rate_limit = 10
    config.rate_window = 60.0
...
def _credential_list_for(descriptor: ProviderDescriptor, base_credential: str) -> list[str]:
    import os
    import re
    keys = []
    
    def _add_keys(val: str):
    ...
    _add_keys(base_credential)
    
    if descriptor.credential_env:
        prefix = f"{descriptor.credential_env}_"
        try:
            ...
            for env_file in _env_files():
                if env_file.is_file():
                    ...
                    for k, v in values.items():
                        if k == descriptor.credential_env or k.startswith(prefix):
                            _add_keys(v)
        except Exception:
            pass
...
def _credential_for(descriptor: ProviderDescriptor, settings: Settings) -> str:
    ...
    if descriptor.credential_attr:
        val = _string_attr(settings, descriptor.credential_attr)
    if not val:
        keys = _credential_list_for(descriptor, "")
        if keys:
            val = keys[0]
    return val
...
def build_provider_config(
    ...
    keys = _credential_list_for(descriptor, credential)
    if not credential and keys:
        credential = keys[0]
    ...
    rate_limit = settings.provider_rate_limit if settings.provider_rate_limit is not None else descriptor.default_rate_limit
    rate_window = settings.provider_rate_window if settings.provider_rate_window is not None else descriptor.default_rate_window
...
def create_test_provider(provider_id: str, api_key: str, base_url: str | None, settings: Settings) -> BaseProvider:
    """Create an ephemeral provider instance for testing an API key/endpoint."""
    descriptor = PROVIDER_DESCRIPTORS.get(provider_id)
    if descriptor is None:
        supported = "', '".join(PROVIDER_DESCRIPTORS)
        raise UnknownProviderTypeError(
